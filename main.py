...
carro_loc = visual.posicionar_carro(carro, visual.FAIXA_DIREITA, (80/100 * visual.JANELA_ALTURA) )
carro2_loc = visual.posicionar_carro(carro2, visual.FAIXA_ESQUERDA, (20/100 * visual.JANELA_ALTURA))
  ...
  for event in pygame.event.get():
    ...
    if event.type == KEYDOWN:
      if event.key == K_a or event.key == K_LEFT:
        mecanica.alternar_faixa(carro_loc, mecanica.FAIXA_ESQUERDA)
      if event.key == K_d or event.key == K_RIGHT:
        mecanica.alternar_faixa(carro_loc, mecanica.FAIXA_DIREITA)
